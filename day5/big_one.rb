...
post '/newshout' do
	...
	passtoshout = params[:passtoshout]
	user = User.find(params[:user_id])
	if  passtoshout == user.password
		shouts_id += 1
		newshout = Shout.new
	...
	session[:warning] = "Wrong pasword! Try again"
	end
end
...
post '/likes' do 	
	#likes = params[:like]
	#likes += 1
	get_selected_shout = Shout.find_by_id(params[:likes])
	get_selected_shout.likes +=1
	get_selected_shout.save
	redirect '/'
